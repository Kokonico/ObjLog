...
log.log(
    Info(
        f"took {time.time() - wait_queue_start_time:.2f} seconds to write all asynchronous messages to disk post-queue"
    )
)
log.log(
    Info(
        (
            ...
            else "asynchronous" if async_count > sync_count else "both"
        )
        + f" was the faster mode with a difference of {abs(async_count - sync_count)} messages more logged in 1 second (percentage difference: {abs(async_count - sync_count) / max(sync_count, async_count) * 100:.2f}% faster)"
        if sync_count != async_count
        else f" modes logged the same number of messages in 1 second ({sync_count} messages)"
    )
)
