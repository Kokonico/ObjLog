...
class TestLogNode(unittest.TestCase):
    ...
    def setUpClass(cls):
        ...
        cls.log = LogNode(name="Test", log_file=os.path.join(LOG_FOLDER, "LogNodeTest.log"), log_when_closed=False, asynchronous=True)
        cls.log2 = LogNode(name="Test2", log_file=os.path.join(LOG_FOLDER, "LogNodeTest2.log"), log_when_closed=False, asynchronous=True)
        cls.log_at_console = LogNode(name="Test", print_to_console=True, asynchronous=True)
        cls.log_at_none = LogNode(name="Test", asynchronous=True)
    ...
    def tearDown(self):
        ...
        self.log2.await_finish()
        self.log_at_console.await_finish()
        self.log_at_none.await_finish()
        self.log.enable()
        self.log2.enable()
    ...
    def test_lgnd_save_and_load(self):
        ...
        self.log.await_finish() # only check after it actually finished saving
