...
class TestLogNode(unittest.TestCase):
    ...
    def test_lognode_get(self):
        ...
        for message in messages:
            self.log.log(message)
        self.assertListEqual(self.log.get(element_filter=[Debug, Info, Warn, Error, Fatal, CustomMessage]), messages)
        self.tearDown()
    ...
    def test_lognode_squash(self):
        ...
        self.assertEqual(squash_message, self.log.get(element_filter=[Debug, Info, Warn, Error, Fatal, CustomMessage])[0])  # check that the first message is the squash message
        self.tearDown()
    ...
    def test_lognode_combine(self):
        ...
        self.assertListEqual(self.log2.get(element_filter=[Debug, Info, Warn, Error, Fatal, CustomMessage]), messages)
        self.tearDown()
    ...
    def test_lognode_filter(self):
        ...
        for message in self.log.get(element_filter=[Debug, Info, Warn, Error, Fatal, CustomMessage]):
            self.assertTrue(isinstance(message, CustomMessage))
        self.tearDown()
